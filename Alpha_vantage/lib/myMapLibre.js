...
/* Elenco dei metodi esposti :
   ...
   2. async drawMap(urlStyle, mapContainer, gpsAddress, zoom) 	
   3. addPOILayer(ulrStyle)   
...
async addMarker (gpsAddress, icon, label, popupText) {
    ...
    const popup = new maplibregl.Popup({ offset: [60, -20] })
    if (popupText)
    ...
    div.className = 'custom-marker'; // custom CSS class   
    // oppure
    ...
    if(label){
        ...
        span.style.top = '-20px';   // 20px verso l'alto 
    ...
    // Funzione che controlla lo zoom e mostra/nasconde il marker
    ...
    // Eseguiamo il controllo iniziale per assicurarci che lo stato sia corretto fin dall'inizio
    toggleMarkerVisibility();

    // Aggiungiamo un listener sull'evento "zoom" della mappa
    this.map.on('zoom', toggleMarkerVisibility);
}
